	   ...
	   DSPCMPLX * out, double *upar)
{
  ...
  /*for (k = 0; k < 7; k++)
    //printf(" %d", dpar[k]);
  printf("\n");*/
  deco->nbits = dpar[0]; //bauds per loop
  nr_ipp = dpar[1];  //the slow time sampling
  vec_len = dpar[2];  //the fast time sampling
  deco->nfract = dpar[3];  //sampling number per baud
  deco->maxlag = (dpar[4] - deco->nfract + 1) / deco->nfract;///the integer number of lags
  sub_int = dpar[5];  //1
  nr_code_sets = dpar[6];  //loops per circle
  if (nr_code_sets < 0)
    {
      nr_code_sets *= -1;
  ...
  deco->decomode = DSP_TAILDECO; //decomode=2
  deco->zprflen1 = (deco->nbits - 1) * deco->nfract + vec_len - 1;
  deco->rmsa = 0;
  deco->fstlag = 1;
  nlags = deco->maxlag * deco->nfract + deco->nfract - 1; // lags number
  deco->ndeco = __user__decolen (vec_len, nlags, deco->nbits, deco->nfract,
				 deco->decomode);  //data number
		  ...
		  DACCINFO * info)
{
  ...
  if (deco->nfract < 2) //the lag interval is one baud
    {				/*integer-only lags */
    ...
    {
      if (deco->decomode == DSP_TAILDECO) //add zeros decode
	{
      ...
      else if (deco->decomode == DSP_ACDECO) // don't add zeros decode
	{
  ...
  for (lag = deco->fstlag; lag <= deco->maxlag; lag++)
    {
      ...
      __user__getfilter (decofir, &ntaps, deco->code, lag, deco->nbits);  // get the decode code for this lag
		       ...
		       ulong max_lag, ulong lag_incr, int zer_away, int loops)
{
  int j, n, k, m, lp;
  //mn=0;
  double a, b, c, d, tmpr, tmpi;
  DSPCMPLXSHORT *datat;
 // printf("datain=%f,d2=%f\n",(float)(data[0].re),(float)(data[0].im));
  msize /= loops;
  for (lp = 0; lp < loops; lp++)
    {
      datat = data + lp * msize;

     // printf("r1=%f,r2=%f\n",(double)(datat)->re,(double)(datat)->im);
      for (j = 0; j < 2 * (msize - max_lag + 1); j += 2) //calculation for the lag 
	{
	  a = (double) (datat + j / 2)->re;//
	  b = (double) (datat + j / 2)->im;//
	  for (n = -2 * msize + j - 2 * lag_incr * zer_away, k = 0;
	       k < max_lag * 2; k += 2 * lag_incr)
	    {
	      c = (double) (datat + (j + k) / 2)->re;//
	      d = (double) (datat + (j + k) / 2)->im;
	    ...
	    if (j==0&& k==0)
               {
               //mn=mn+1;
               //printf("a=%f,b=%f,c=%f,d=%f,tmpr=%f,tmpi=%f\n",a,b,c,d,tmpr,tmpi);
               }
            }
	}
...
__user__decoder_thread (DECO * deco_send)
{
 ...
 ////////calculate the sp
  for (k = 0; k < (nr_ipp / nr_code_sets) / sub_int; k++) //integration circle
    {
      for (i = 0; i < sub_int; i++) //transmit frequency circle
	{
	  in_data =
	    in + j * vec_len * sub_int +
	    k * vec_len * sub_int * nr_code_sets + i * vec_len;  // the position of the input
          if (k==0)
              {
                //printf("j=%d,",j);
                //printf("datain=%f,d2=%f\n",(float)(in_data[0].re),(float)(in_data[0].im));
              }
	  __user__lag_profiling (in_data, lag_res1, nsamples, nlags + 1, 1,
				 0, 1);
	}
    }
/////////calculate the sp2, add zeros 
  to = lag_res2;
  ...
  if (j == nr_code_sets - 1)
    ...
    deco_send->accum = 1;
  deco_send->code = deco_send->dpar + 7 + j * deco_send->nbits; // the transmit code
/////////calculate the sp3, decoded
...
void
pyface (int *par, int nin, DSPCMPLX * in_r, DSPCMPLX * out)
{
  ulong nb = par[0] + par[4] * par[7]; //16+47*1
  float *fpar = NULL,*upar = NULL;
  int i;
  DSPCMPLXSHORT *in;//
  in = (DSPCMPLXSHORT *) malloc (nin * sizeof (DSPCMPLXSHORT)); //give the storage for the input 
  for (i = 0; i < nin; i++) //read the raw IQ
    {
      in[i].re = in_r[i].re;
      in[i].im = in_r[i].im;
    }
  //printf("in1=%d,in2=%d\n",(in[0].re),(in[0].im));
  alter_dec (nb, par, 0, fpar, in, out, upar);
  //printf("!!!!!out=%lf,%lf",(out[1].re),(out[2].im));
  free (in);
}
