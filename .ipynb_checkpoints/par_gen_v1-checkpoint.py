...
####add for syisr by junyi
def accgen(code_len, code_tx,nr_codes):
	ac_code=[]
	code_fil='AC%d.txt'%code_len	#File AC16.txt with transmit code
	ac_code=[]
	lines=open(code_fil,'r').readlines()
	ll=range(len(lines))
	for l in ll:
...
def acdecgen(exp_name,ac_code,code_tx,nr_loop,loops,isamp,ion_frac,ion_lag):
	...
	pars = [code_tx, nr_loop * loops, isamp, ion_frac, ion_lag, 0, loops]
